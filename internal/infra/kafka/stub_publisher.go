...
// PublishSessionVersionBumped logs iam.session.version.bumped events.
func (p *StubPublisher) PublishSessionVersionBumped(_ context.Context, event domain.SessionVersionBumpedEvent) error {
	payload := map[string]any{
		"session_id": event.SessionID,
		"user_id":    event.UserID,
	...
	p.logEvent("iam.session.version.bumped", event.UserID, event.BumpedAt, payload)
	return nil
}
